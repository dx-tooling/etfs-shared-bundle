...
final class OnlyInterfacesInDomainServiceConstructorsRuleTest extends TestCase
{
    public function testReportsErrorWhenConcreteParamInDomainService(): void
    {
        $root    = realpath(__DIR__ . '/../../../..');
        $fixture = $root . '/tests/Fixtures/PhpStan/DomainServiceInterfaceOnly/Bad/BadDomainService.php';

        [$exitCode, $errors] = $this->runPhpStan([
            'EnterpriseToolingForSymfony\\SharedBundle\\PhpStan\\Rules\\OnlyInterfacesInDomainServiceConstructorsRule',
        ], $fixture);

        $this->assertSame(1, $exitCode);
        $this->assertNotEmpty($errors);
        $this->assertStringContainsString('must depend on interfaces only', (string) ($errors[0]['message'] ?? ''));
    }

    public function testPassesWhenUsingInterfaceInDomainService(): void
    {
        $root    = realpath(__DIR__ . '/../../../..');
        $fixture = $root . '/tests/Fixtures/PhpStan/DomainServiceInterfaceOnly/Good/GoodDomainService.php';
